# FDE 所需工具地图 · 一个 AI-native FDE 团队的操作系统

> 整理日期：2026-06-30 · 数据来自 GitHub 实查（star/许可证为 2026 年中近似值，对外分发前以仓库 LICENSE 为准）
> **视角：不是"一个 FDE 个人用什么工具"，而是"一个 AI-native FDE 团队如何运转"。**
...
> **"团队"是一个光谱，不是固定编制。** 最小形态就是 **1 个人 + N 个 agent**——一个 3C 全占的 FDE 独自编排一支 agent 舰队，单人即整队；往上是 **多人 + 多 agent**——几个互补的人各自带一队 agent 协作。衡量单位不是人头，而是"**人类判断核心 × agent 舰队**"。下面所有角色，既可以由不同的人分担，也可以由同一个人在不同阶段切换戴不同的帽子，由 agent 补齐其余产能。

---

# 🤖 第一部分：团队的组成（人 + agent 舰队）

## 1. 人的核心角色（保持精简，只占 3C 里别人补不上的那一面）
...
|---|---|---|---|
| **单兵（1 人 + N agents）** | 1 个 3C 全占的 FDE，一人兼全部角色 | 重度依赖：SDR/discovery/coding/ops/support 全交给 agent | SMB / 早期项目；直接对客户 CEO |
